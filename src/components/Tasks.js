...
const Tasks = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [tasksRes, projectsRes] = await Promise.all([
        axios.get('https://team-gamma-gold.vercel.app/api/tasks', config),
        axios.get('https://team-gamma-gold.vercel.app/api/projects', config)
      ]);
      ...
      if (currentUser.role === 'Admin') {
        const usersRes = await axios.get('https://team-gamma-gold.vercel.app/api/users', config);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('https://team-gamma-gold.vercel.app/api/tasks', taskData, {
  ...
  const updateTaskStatus = async (taskId, newStatus) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`https://team-gamma-gold.vercel.app/api/tasks/${taskId}/status`, 
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDeleteTask = async (taskId) => {
    if (window.confirm('Are you sure you want to delete this task?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://team-gamma-gold.vercel.app/api/tasks/${taskId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
